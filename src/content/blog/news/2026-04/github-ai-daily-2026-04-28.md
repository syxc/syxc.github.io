---
pubDatetime: 2026-04-28T09:00:00+08:00
title: GitHub AI 日报 - 2026-04-28
slug: github-ai-daily-2026-04-28
featured: true
draft: false
description: "GitHub AI Daily - AI 项目趋势分析"
tags:
  - AI
  - GitHub
  - Trending
  - News
---

今日共发现 8 个 AI 相关热门项目。

### mattpocock/skills

- Stars: +2,565 this week
- Language: Shell
- Description: Skills for Real Engineers. Straight from my .claude directory.
- Link: [mattpocock/skills](https://github.com/mattpocock/skills)

### abhigyanpatwari/GitNexus

- Stars: +3,653 this week
- Language: TypeScript
- Description: GitNexus: The Zero-Server Code Intelligence Engine - GitNexus is a client-side knowledge graph creator that runs entirely in your browser. Drop in a GitHub repo or ZIP file, and get an interactive knowledge graph wit a built in Graph RAG Agent. Perfect for code exploration
- Link: [abhigyanpatwari/GitNexus](https://github.com/abhigyanpatwari/GitNexus)

### Alishahryar1/free-claude-code

- Stars: +2,335 this week
- Language: Python
- Description: Use claude-code for free in the terminal, VSCode extension or via discord like openclaw
- Link: [Alishahryar1/free-claude-code](https://github.com/Alishahryar1/free-claude-code)

### gastownhall/beads

- Description: Beads - A memory upgrade for your coding agent
- Link: [gastownhall/beads](https://github.com/gastownhall/beads)

### davila7/claude-code-templates

- Description: CLI tool for configuring and monitoring Claude Code
- Link: [davila7/claude-code-templates](https://github.com/davila7/claude-code-templates)

### microsoft/VibeVoice

- Description: Open-Source Frontier Voice AI
- Link: [microsoft/VibeVoice](https://github.com/microsoft/VibeVoice)

### TauricResearch/TradingAgents

- Description: TradingAgents: Multi-Agents LLM Financial Trading Framework
- Link: [TauricResearch/TradingAgents](https://github.com/TauricResearch/TradingAgents)

### deepseek-ai/DeepSeek-V3

- Link: [deepseek-ai/DeepSeek-V3](https://github.com/deepseek-ai/DeepSeek-V3)
